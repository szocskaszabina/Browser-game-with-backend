...
//Set default values
    ...
    document.getElementById('hanging').src = 'pics/bg.png';
    document.getElementById('details').innerHTML = '';
    document.getElementById('guess').value = '';
    document.getElementById('mistakes').innerHTML = 'Wrong letters:';
    document.getElementById('sendFormBtn').classList.add('d-none');
    mistakeCounter = 0;
...
guessButton.addEventListener('click', (e) => {
    ...
    for (let element = 0; element <= task.length; element++ ) {
        ...
        if(!task.includes(guessLetter)) {
                ...
                document.getElementById('hanging').src = `pics/bg${mistakeCounter}.png`;
...
//Send form to save movie to database

function fillFormToSaveMovieToDB() {
    ...
    document.getElementById('sendFormBtn').classList.remove('d-none');
}
//Send form automatically when user wins to update user score

async function updateUserScore(points) {

    let user_id =  document.getElementById('score').value;
    let url = `/game-app/public/user/${user_id}/add_score`;
    let response = await fetch(url, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-Token': document.getElementsByName('_token')[2].value,
        },
        body: points
    })
    await response.json();
